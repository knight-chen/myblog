...
<html>
    <head>
        ...
        <script>
            var point_msg = '';
            var url = "<?php echo U('Daily/update_draft_type','','',0);?>";
            function back(url) {
                window.location = url;
            }
        </script>
                    ...
                    <tr>
                        <td>
                            <div>
                                ...
                                <form action="/blog/index.php/Admin/Daily/update_draft/draftId/3" method="post" name="adddraft" id="adddraft">
                                    <input type="text" name="draft[name]" id="draft_name" placeholder="日志标题" value="<?php echo ($draftInfo['name']); ?>"/>
                                    <input type="hidden" id="draft_url" value="<?php echo U('Daily/sava_to_draft');?>"/>
                                    <li style="none" id="li">
                                        <span style="width:100px;"><b>日志类别：</b></span>
                                        <select name='draft[typeid]' id='draft_type'>
                                            <?php if(!empty($draftInfo['type'])): ?><option value="<?php echo ($draftInfo['typeid']); ?>"><?php echo ($draftInfo['type']); ?></option><?php endif; ?>
                                            <?php if(is_array($typeInfo)): foreach($typeInfo as $k=>$v): if($v['id'] != $draftInfo['typeid']): ?><option value="<?php echo ($v['id']); ?>"><?php echo ($v['type_name']); ?></option><?php endif; endforeach; endif; ?>
                                        </select>
                                        <span id='c_time' style=" margin-left: 100px !important; font-size: 14px;"><b>修改日期：</b></span><input type="text" style="height: 21px; " id="ctime" name="draft[c_time]" readonly="true" value="<?php echo date('Y-m-d',time());?>"/>
                                        <span style=" margin-left: 100px; width:100px;"><b>权限设置：</b></span>
                                        <select style="width: 100;height: 25px;" name='draft[permission]' id='permission'>
                                            <?php if(!empty($draftInfo['permission'])): ?><option value="<?php echo ($draftInfo['permissionid']); ?>"><?php echo ($draftInfo['permission']); ?></option><?php endif; ?>
                                            <?php if(is_array($permission)): foreach($permission as $key=>$v): if($v['id'] != $draftInfo['permissionid']): ?><option value="<?php echo ($v['id']); ?>"><?php echo ($v['permission']); ?></option><?php endif; endforeach; endif; ?>
                                        </select>
                                        <span style='margin-left: 100px !important;font-size: 14px;'><b>原创作者：</b></span><input type='text' style="height: 21px; width: 160px; " id='author' name='draft[author]' value="<?php echo ($draftInfo['author']); ?>"/>
                                    </li>
                                    <input type='hidden' id='draft_content' value="<?php echo ($draftInfo['content']); ?>"/>
                                    <script id="container" name="draft[content]" type="text/plain">
                                    </script>
                                    ...
                                    <li style=" margin-top: 6px;">
                                        ...
                                        <input type="button" name="btn_draft" id="btn_draft" value="存为草稿" style="width: 66px;height: 31px;background-color:azure"/>
                                    </li>
                                </form>
                            </div> 
                        </td>
